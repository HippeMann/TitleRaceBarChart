...
qual.colors.list <- list(
  ...
  'England-Premier League' = c("4" = "#1A7000", "7" = "#005770", "17" = "#F03629"), 
  'Spain-LaLiga' = c("4" = "#1A7000", "7" = "#005770", "17" = "#F03629"), 
  'Italy-Serie A' = c("4" = "#1A7000", "6" = "#0094BE","17" = "#F03629"), 
  'Germany-1. Bundesliga' = c("4" = "#1A7000", "7" = "#005770", "15" = "#FF911B", "16" = "#F03629"), 
  'France-Ligue 1' = c("2" = "#1A7000",  "3" = "#2DBE00", "4" = "#005770", "17" = "#FF911B", "18" = "#F03629"), 
  'Portugal-Liga Nos' = c("2" = "#1A7000",  "5" = "#0094BE","15" = "#F03629"), 
  'Netherlands-Eredivisie' = c("2" = "#2DBE00", "7" = "#0094BE", "15" = "#FF911B", "17" = "#F03629"), 
  'England-Championship' = c("2" = "#1A7000", "6" = "#2DBE00", "21" = "#F03629"),
  'Russia-Premier Liga' = c("2" = "#1A7000", "3" = "#2DBE00", "5" = "#0094BE", "12" = "#FF911B", "14" = "#F03629"), 
  'Turkey-Super Lig' = c("1" = "#1A7000", "2" = "#2DBE00", "4" = "#0094BE","15" = "#F03629"), 
...
minPoints <- min(dt$Points)
maxPoints <- max(dt$Points)
...
TeamColors.csv <- read.csv(file="TeamColors.csv", header=T, stringsAsFactors = F)
TeamColors <- TeamColors.csv[,2]
names(TeamColors) <- TeamColors.csv[,1]
...
p <- dt %>% #filter(Matchday == max.matchday) %>%
  ...
  geom_image(image = league.logo, x = n.teams / 2, y = maxPoints, size = 0.2) + #League Logo at the top center, add + 1 to the x to nudge it for the Bundesliga
  ...
  #Colors
  scale_fill_manual(values = TeamColors) + #colors of the bars
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, maxPoints + 10), breaks = seq(0, maxPoints,3)) + #minPoints-10
  #Theme
  theme_classic() + 
...
p <- dt %>% #filter(Matchday == max.matchday)
  ...
  geom_image(image = league.logo, x = n.teams / 2, y = maxPoints, size = 0.2) + #League Logo at the top center
  #Vertical "qualification" lines done manually so as to not go below y = 0
  annotate('segment', x = 4 + 0.5, xend = 4 + 0.5, colour = "#1A7000", y = 0, yend = maxPoints + 10, alpha = 0.5, size = 1.5) + 
  annotate('segment', x = 7 + 0.5, xend = 7 + 0.5, colour = "#005770", y = 0, yend = maxPoints + 10, alpha = 0.5, size = 1.5) + 
  annotate('segment', x = 17 + 0.5, xend = 17 + 0.5, colour = "#F03629", y = 0, yend = maxPoints + 10, alpha = 0.5, size = 1.5) + 
  #Colors
  scale_fill_manual(values = TeamColors) +
  #Scales
  scale_x_continuous(expand = c(0, 0), limits = c(0, n.teams + 0.5), breaks  =seq(1, 24, 1)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(minPoints - 10, maxPoints + 10), breaks = seq(0, maxPoints, 3)) + 
  #Theme
  theme_classic() + 
  theme(legend.position = "none",
        plot.title = element_text(size=36),
